...
public class SettingsController implements Screen {
    ...
    private SettingsView settingButtons;
    private Music music;
    ...
    public SettingsController(JumpJellyJump game) {
        ...
        settingButtons = new SettingsView(game.batch);
    ...
    public void render(float delta) {
        ...
        if (settingButtons.isVolumeDownPressed()) {
            JumpJellyJump.setVolumeLevel(JumpJellyJump.getVolumeLevel() - 0.01f);
            music.setVolume(JumpJellyJump.getVolumeLevel());
            dispose();
        } else if (settingButtons.isVolumeUpPressed()) {
            JumpJellyJump.setVolumeLevel(JumpJellyJump.getVolumeLevel() + 0.01f);
            music.setVolume(JumpJellyJump.getVolumeLevel());dispose();
            dispose();
        } else if (settingButtons.isVolumeMutePressed()) {
            JumpJellyJump.setVolumeLevel(0f);
            music.setVolume(JumpJellyJump.getVolumeLevel());
            dispose();
        } else if (settingButtons.isBackPressed()) {
        ...
        game.batch.setProjectionMatrix(settingButtons.stage.getCamera().combined);
        settingButtons.stage.draw();
    }

    public void update(float delta) {
        settingButtons.update(delta);
    }
    ...
    public void resize(int width, int height) {
        ...
        settingButtons.resize(width, height);
    }
